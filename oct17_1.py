...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QPushButton, QFileDialog, QLabel, QComboBox, QTextEdit, QHBoxLayout
...
def plot_measurements(tracks, x_attr, y_attr):
    ...
    for track in tracks:
        ...
        x_data = locals()[x_attr]
        y_data = locals()[y_attr]

        ax.plot(x_data, y_data, label=f'Track {track["track_id"]} {y_attr} vs {x_attr}', marker='o')

    ax.set_xlabel(x_attr)
    ax.set_ylabel(y_attr)
    ax.set_title(f'Tracks {y_attr} vs {x_attr}')
    ax.legend()
...
def main(input_file, track_mode, filter_option, association_type):
    ...
    plot_measurements(tracks, 'times', 'measurements_x')
...
class KalmanFilterGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.track_mode_label = QLabel("Select Track Initiation Mode")
        ...
        main_layout.addWidget(self.track_mode_label)
        main_layout.addWidget(self.track_mode_combo)

        # Filter option selection
        self.filter_option_label = QLabel("Select Filter Option")
        ...
        main_layout.addWidget(self.filter_option_label)
        main_layout.addWidget(self.filter_option_combo)

        # Association type selection
        self.association_type_label = QLabel("Select Association Type")
        ...
        self.x_axis_label.setFont(QFont('Arial', 10))
        self.x_axis_combo = QComboBox()
        self.x_axis_combo.addItems(["times", "measurements_x", "measurements_y", "measurements_z", "Sf_x", "Sf_y", "Sf_z"])
        self.x_axis_combo.setFont(QFont('Arial', 10))
        main_layout.addWidget(self.x_axis_label)
        main_layout.addWidget(self.x_axis_combo)

        # Y-axis selection
        self.y_axis_label = QLabel("Select Y-axis")
        self.y_axis_label.setFont(QFont('Arial', 10))
        self.y_axis_combo = QComboBox()
        self.y_axis_combo.addItems(["times", "measurements_x", "measurements_y", "measurements_z", "Sf_x", "Sf_y", "Sf_z"])
        self.y_axis_combo.setFont(QFont('Arial', 10))
        main_layout.addWidget(self.y_axis_label)
    ...
    def process_data(self):
        ...
        # Call the main processing function with the selected options
        main(input_file, track_mode, filter_option, association_type)
...
class PlotWindow(QWidget):
    ...
    def plot(self):
        ...
        x_attr = self.parent().x_axis_combo.currentText()
        y_attr = self.parent().y_axis_combo.currentText()
        plot_measurements(self.parent().tracks, x_attr, y_attr)
